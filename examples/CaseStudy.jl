...
# binhkorn_vars = [ADOPT.RealVariable(0, 5), ADOPT.RealVariable(0, 3)]
#
# binhkorn_f1(x) = 4x[1]^2 + 4x[2]^2
# binhkorn_f2(x) = (x[1] - 5)^2 + (x[2] - 5)^2
# binhkorn_objs = [ADOPT.Objective(binhkorn_f1), ADOPT.Objective(binhkorn_f2)]
#
# binhkorn_c1(x) = (x[1] - 5)^2 + x[2]^2 - 25 # <= 0
# binhkorn_c2(x) = (x[1] - 8)^2 + (x[2] - 3)^2 - 7.7 # >= 0
# binhkorn_cnstrs = [ADOPT.Constraint(binhkorn_c1, <=), ADOPT.Constraint(binhkorn_c2, >=)]
#
# binhkorn = ADOPT.Model(binhkorn_vars, binhkorn_objs, binhkorn_cnstrs)
# maxevals = 1000
# nparticles = 100
# PSO_params = Dict(:leader_size => nparticles, :warm_size => nparticles)
# ADOPT.benchmark(nruns=1, algorithms=[(SMPSO, PSO_params)], problem=binhkorn, max_evals=maxevals)
# -------------------------------------------------------------------------
# Evolutionary Solver
# -------------------------------------------------------------------------
