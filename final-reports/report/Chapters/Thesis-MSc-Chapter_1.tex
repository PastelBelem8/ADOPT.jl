...
Despite the possibility of automating optimization processes, the lack of standardized optimization approaches still comprises an obstacle towards its application within the architectural community~\cite{Attia2013}. In particular, to use such optimization processes, architects often need to acquire programming skills either to create their own optimization algorithm or to integrate dedicated optimization software (e.g., MATLAB toolbox and GenOpt) into their program. To this end, not only do architects need to have optimization- and programming-specific knowledge, but they are also required to invest large amounts of time and effort. Moreover, such optimization tools rarely provide post-processing and visualization mechanisms to aid in the interpretation of the obtained results, which prompts architects to perform additional efforts to integrate these features into their optimization process (e.g., using DView, Excel, or gnuplot). As a result of the additional efforts involved in the implementation of such automated processes, architects tend to discard optimization or, when that is not the case, to repeatedly adopt the same algorithm. While this is not always a disadvantage, it can have a drastic impact in the overall optimization time if, for example, architects systematically apply algorithms with bad performance. 
...
%\todo{Bruno - sinto a falta de um pequeno parágrafo a concluir esta secção e a transitar para a secção 1.2}
% #############################################################################
...
\textbf{\Cref{chap:architecture}} describes the architecture of the proposed framework and enumerates important design decisions that were made during its implementation. \\
\textbf{\Cref{chap:implement}} describes the application of the proposed framework to the architectural practice. \\
\textbf{\Cref{chap:evaluation}} analyzes both quantitative and qualitative aspects of the proposed solution, evaluating its performance in the context of three real case studies. \\
\textbf{\Cref{chap:conclusion}} emphasizes the importance of optimization in architecture and draws some conclusions about this work and how it can influence the architectural practice. Finally, we reflect on the future improvements for the proposed framework. \\
