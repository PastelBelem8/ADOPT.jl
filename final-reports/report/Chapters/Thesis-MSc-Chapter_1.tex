...
\label{chap:intro}
	...
	The application of optimization goes beyond day-to-day life decisions (e.g., buying a car or a computer), also having a paramount impact on decisions in the fields of economy, science, mechaninis, engineering, among others. As a case in point, optimization yields a great potential to the architecture field, as it directly impacts the building industry: optimization enables the reduction of the economic and ecological footprint of the building sector through the finding of more efficient building variants, prior to their construction. 
	...
	Despite the benefits attained with \ac{PBD} and existing optimization approaches, these still involve several manual decisions by engineers and architects. Given its importance to the world's sustainability and economy, this dissertation attempts to automate the \ac{PBD} process and, thus, enhance the architectural practice. To this end, we propose a framework that considers the whole design process to enable the simple application of optimization algorithms and the creation of optimized designs in architecture. Typical design processes involve a design phase, where the architect creates a changeable design, an analysis stage, where analysis tools may be used to evaluate the design's performance, and an optimization stage, where optimization algorithms can be used to find the best performing design variations.
...
\textbf{\Cref{chap:architecture}} describes the architecture of the implemented framework and enumerates important design decisions that were made during its implementation. \\\todo{ADD one chapter about the implementation...}
