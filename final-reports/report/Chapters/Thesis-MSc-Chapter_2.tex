...
\label{sec:optimizationalgorithms}
	...
	Mathematically speaking, optimization algorithms can be classified differently according to their properties \cite{Koziel2011, Nocedal2011NumericalOptimization}. \Cref{fig:optALGOclassification} presents a schematic representation of the classifications considered in this dissertation. For a more comprehensive treatment of these classifications.
	...
	The multidisciplinary aspect of building design raises distinct problems, ranging from the well-behaved with simple, unimodal, convex functions, to the ill-behaved with irregular, multimodal objective functions~\cite{Wortmann2017ADO}. In addition to problem's diversity, the time complexity associated with function evaluations also becomes an important factor to consider.
		\begin{table}[]	
		\centering	
		\resizebox{\textwidth}{!}{%	
	...
	Another problem emerges from the fact that a specific optimization algorithm may perform well for some problems and have bad performance in other problems~\cite{Wortmann2017GABESTCHOICE, Fang2017}. This idea resembles the one captured in Wolpert's \ac{NFLT} for optimization, stating that for any algorithm, better
	...
	Inevitably, the same building design description might yield different problem descriptions according to the performance aspects being considered. Some algorithms might explore certain descriptions more effectively than others, e.g., because the objective functions describing the lighting and structural behavior of a certain design may have completely different properties. In an attempt to exploit this fact, \ac{BPO} practitioners should dedicate a small amount of their total time budget to test various algorithms and different setup parameters, before finally settling for an optimization algorithm~\cite{Hamdy2016}.	
	...
	\begin{itemize}
		...
		\item a similarity representation graph that represents solutions that are genetically similar to each other, and marks solutions that contribute to the creation of the next generation with black dots, whilst non-contributors are marked with a red cross. Exhibited for the \textit{\ac{GA}} solver;
	...
	\begin{figure*}[]
		...
		\caption[Galapagos GUI]{ Three views of the Galapagos' \ac{GUI}: (a) Solvers configuration menu (b) Graphical views for the \textit{\ac{GA}} solver (c) Graphical views for the \textit{Simulated Annealing} solver}
		\label{fig:galapagos}
	\end{figure*}
...
%		}
	...
	The textual paradigm does not face these scalability issues. Most currently existing textual \ac{AD} tools do not support optimization, but already provide the mechanisms to create automated optimization processes.
