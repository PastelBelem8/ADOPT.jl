...
\label{chap:back}
	...
	Firstly, the \textit{ad-hoc} nature of the functions used for performance assessment in \ac{BPO} motivates the application of a special class of optimization algorithms, the derivative-free algorithms. Within this class, different categories emerge, emphasizing the algorithms' different properties and search strategies. Applying the adequate algorithm to a certain problem may potentially increase the efficiency of an optimization process, especially for problems involving time-consuming evaluations.
	
	Secondly, there are multiple approaches to optimization that might be considered. Generally, \ac{BPO} practices require the optimization of multiple aspects simultaneously. However, architects often opt for simpler specifications, often disregarding all but one of the initially considered aspects. 
...
\label{sec:optimizationalgorithms}
	...
	The third classification distinguishes optimization in terms of the search aims, particularly, whether it is a \textbf{global} or \textbf{local} search. In local optimization, the search strategy strives to find a solution that is locally optimal, i.e., for which its value is better than all other points in its vicinity. In this case, the points that satisfy that property are known as local optima. Conversely, global optimization strives to find the globally optimal solutions, i.e., the best of all the local optima.	
