...
\label{sec:optimizationproblems}
	...
	\todo{REVER}
	In order to perform optimization it is first necessary to formulate the model of a problem, a process known as modeling. In the field of mathematic optimization, such formulations generally take the form represented in \cref{eq:genericopt}, where $x = (x_1\,, x_2\,, \ldots\,, x_n)$ represents the vector of parameters (or variables) of the problem, whose values are to be changed during optimization, $f_i$ represents the $i$\textit{th} objective function, whose value is to be optimized (\cref{eq:genericopt1}), and, finally, $g_j$ and $h_k$ represent functions, whose value can be constrained by some conditions in order to guarantee that the obtained solutions are valid (\cref{eq:genericopt2,eq:genericopt3}) ~\cite{Koziel2011}. 
