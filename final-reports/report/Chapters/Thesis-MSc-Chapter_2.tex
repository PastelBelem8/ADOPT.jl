...
\label{sec:optimizationalgorithms}
	...
	% In addition to the simulations' specificity and complexity, architectural designs are inherently complex, thus leading to less predictable objective functions for which mathematical forms are difficult to formulate~\cite{Machairas2014}. For this reason, information about the derivatives of such hypothetical functions, i.e., the direction and magnitude of the greatest increase of the function, cannot be extracted and, therefore, cannot be used to guide the search towards locally optimal solutions. Consequently, classical gradient-based optimization algorithms cannot be used to address architectural optimization problems because they need the functions' derivatives to find the best way to reach optimal solutions. Instead, other algorithms that do not rely on the existence of an explicit mathematical form should be used, i.e., algorithms that treat the optimization functions as black-boxes, relying uniquely on the outputs of numerical simulations.
