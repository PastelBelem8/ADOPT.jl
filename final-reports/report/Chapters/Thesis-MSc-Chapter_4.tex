...
In this section, we explored the application of a general purpose optimization framework to \ac{BPO} problems, which typically incorporate time-consuming evaluation functions that may take up to several seconds, minutes, hours, or even days to complete. Also, we described how the framework can be coupled with existing algorithmic-based approaches to promote the automation of optimization processes, an approach we named \ac{AO}.
