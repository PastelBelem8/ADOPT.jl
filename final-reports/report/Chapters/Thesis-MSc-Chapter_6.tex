...
Regarding the evaluation of different \acp{MOOA}, it is difficult to measure the quality of each algorithm and to relate it with time. The lack of consensus regarding the best way to assess their quality difficults an objective measure of the real impact of model-based algorithms in time reduction. In this dissertation, we adopted a combination of different performance indicators and Pareto front line plots. \todo{continuar...}
 
 . Indeed, we argue that a combination of MOO performance metrics and Pareto Front plots should be used in order to draw more accurate results. Regarding the metrics themselves, we believe it is better to apply different metrics simultaneously, e.g., some should measure the distribution of the non-dominated solutions across the solution space,
